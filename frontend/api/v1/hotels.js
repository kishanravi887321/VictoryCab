...
export default async function handler(req, res) {
  ...
  try {
    const { id, q, destination_id, max_price, min_stars, sort, limit } = req.query;
    ...
    if (max_price) query = query.lte('price_per_night', parseInt(max_price));
    if (min_stars) query = query.gte('rating_stars', parseInt(min_stars));

    if (sort === 'rating') query = query.order('avg_rating', { ascending: false });
    else if (sort === 'price_low') query = query.order('price_per_night', { ascending: true });
    else if (sort === 'price_high') query = query.order('price_per_night', { ascending: false });
